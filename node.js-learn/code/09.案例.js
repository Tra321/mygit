...
function resolveCSS(htmlStr) {
    ...
    //3.4 调用 fs.writeFile() 方法，将提取的样式，写入到 index.css 的文件
...
function resolveJS(htmlStr) {
    //4.2 使用正则提取出页面中的 <script></script> 标签
    const r2 = regScript.exec(htmlStr)
    //4.3 将提取出来的脚本字符串，做进一步的处理
    const newJS =r2[0].replace('<script>', '').replace('</script>', '')
    //4.4 将处理的结果，写入到 index.js 文件里面
...
function resolveHTML(htmlStr) {
    ...
    //5.3 写入 indexnew.html 这个文件
    fs.writeFile(path.join(__dirname, './files/indexnew.html'), newHTML, function(err) {
